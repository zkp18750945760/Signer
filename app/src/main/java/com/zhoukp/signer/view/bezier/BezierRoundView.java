...
public class BezierRoundView extends View implements ViewPager.OnPageChangeListener {
    ...
    private int default_round_count = 6;   //默认圆球的数量
...
//            bezPos[i] = mWidth / (2 * default_round_count) * (i + 1);
//            xPivotPos[i] = mWidth / (2 * default_round_count) * (i + 1) + mRadius;
        }
    }
...
//        float transX = (nextPos - curPos) * (mWidth / (default_round_count * 2));
